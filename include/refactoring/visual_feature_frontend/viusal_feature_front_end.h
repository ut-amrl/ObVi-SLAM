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a  //
      ...
      for (const auto &feat_id_to_track : visual_features) {
        ...
        if (feature_track.feature_track.feature_observations_.find(
          ...
          VisionFeature feature =
              feature_track.feature_track.feature_observations_.at(
                  max_frame_id);
          ...
          if (!is_feature_added_to_pose_graph) {
            ...
            if ((!input_problem_data.getRobotPoseEstimateForFrame(
              ...
              LOG(WARNING)
                  << "Could not find initial or current pose  estimate "
                     "for robot for frame "
                  << max_frame_id << ", not adjusting initial feature position";
              ...
              Position3d<double> relative_initial_position =
                  getPositionRelativeToPose(
                      init_pose_est, feat_id_to_track.second.feature_pos_);
