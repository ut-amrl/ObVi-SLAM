...
Eigen::Vector3d transformPoint(const pose::Pose3d &transform_to_target_frame,
                               const Eigen::Vector3d &point_to_transform) {
  return convertPoseToAffine(transform_to_target_frame) * point_to_transform;
}

Eigen::Vector2d transformPoint(const pose::Pose2d &transform_to_target_frame,
...
pose::Pose2d addGaussianNoise(const pose::Pose2d &original_pose_2d,
...
pose::Pose2d addRelativeGaussianNoise(const pose::Pose2d &original_pose_2d,
                                      const double &x_std_dev,
                                      const double &y_std_dev,
                                      const double &theta_std_dev,
                                      util_random::Random &rand_gen) {
