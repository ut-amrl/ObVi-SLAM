...
struct SingleTrajectoryObjectMetrics {
  ...
  double median_iou_;
  MetricsDistributionStatistics pos_dev_stats_;
  // This structure is aimed at errors, so we should probably make a new one,
  // but just to capture the distribution for now, we're sticking with the same
  // structure
  MetricsDistributionStatistics iou_stats_;
};
