...
//      LOG(INFO) << "Found feature with id " << feat_and_num_obs.first << " only observed once. Removing from observations list";
  ...
  for (const auto &initial_feature_estimates :
    ...
    if (feature_obs.find(feat_id) == feature_obs.end()) {
      //      LOG(WARNING) << "Found initial feature estimate for feature " <<
      //      feat_id
