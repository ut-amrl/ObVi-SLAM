...
void drawLineOnImage(const vtr::PixelCoord<double> &px1,
                     const vtr::PixelCoord<double> &px2,
                     const std_msgs::ColorRGBA &color,
                     cv_bridge::CvImagePtr &cv_ptr) {
  // TODO verify
  int thickness = 1;
  cv::Point p1((int)px1.x(), (int)px1.y());
  cv::Point p2((int)px2.x(), (int)px2.y());
  cv::line(cv_ptr->image,
           p1,
           p2,
           vtr::convertColorMsgToOpenCvColor(color),
           thickness);
}
...
getFeatureReprojections(
  ...
  for (const auto feat_id_and_feature2d : feat_ids_and_features2d) {
    ...
    vtr::drawTinyCircleOnImage(projected_pixel,
...
int main(int argc, char **argv) {
  ...
  LOG(INFO) << "low_level_features_map size: " << low_level_features_map.size();

  std::vector<vtr::FrameId> all_frame_ids;
  for (const auto &frame_id_and_robot_pose : robot_poses) {
    all_frame_ids.push_back(frame_id_and_robot_pose.first);
  }
  std::sort(all_frame_ids.begin(), all_frame_ids.end());
  for (const vtr::FrameId frame_id : all_frame_ids) {
    debugInitVisualFeaturesByFrameId(FLAGS_debug_output_directory,
                                     frame_id,
                                     camera_intrinsics_by_camera,
                                     camera_extrinsics_by_camera,
                                     images.at(frame_id),
                                     robot_poses.at(frame_id),
                                     initial_feat_positions,
                                     low_level_features_map.at(frame_id));
  }
