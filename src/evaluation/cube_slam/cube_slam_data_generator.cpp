// Detection File Format: FrameId x1 y1 width heigh prob
...
DEFINE_string(cube_slam_data_output_directory,
              "test",
...
int main(int argc, char **argv) {
  ...
  // TODO (Taijing): read them in from some configs
  ...
  if (FLAGS_cube_slam_data_output_directory.empty()) {
  ...
  ros::init(argc, argv, "a_" + node_prefix + "cube_slam_data_generator");
  ...
  fs::path root_output_directory(FLAGS_cube_slam_data_output_directory);
  setupOutputDirectory(FLAGS_cube_slam_data_output_directory, true);       
  ...
  for (const auto frame_id : ordered_frame_ids) {
    ...
    for (const auto &cam_id_and_bboxes : cam_ids_and_bboxes) {
      std::ofstream &ofile = cam_ids_and_detection_ofiles.at(cam_id_and_bboxes.first);
      ...
      for (const auto &bbox : cam_id_and_bboxes.second) {
        ofile << frame_id << " " 
          ...
          << bbox.pixel_corner_locations_.second.y() - bbox.pixel_corner_locations_.first.y() << " "
          << bbox.detection_confidence_ << std::endl;
      }
    }
  }
