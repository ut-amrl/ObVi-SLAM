...
class ORBSLAM3SessionConfig:
    def __init__(self, top_level_orb_slam_results_dir, rosbagDirectory, bagIdxInSequence, rosbagBaseName, generate_map_file=False):
...
def generateResultsFileNamesForBagInSeq(bagNumInSeq, bagName, top_level_orb_slam_results_dir):
    ...
    sessionRootDir = FileStructureUtils.ensureDirectoryEndsWithSlash(top_level_orb_slam_results_dir) + "/" + bagSpecificDir
...
def runOrbSLAMSingleSeqAfterSetup(orbSlamSessionConfig, saveTrajService):
    ...
    time.sleep(15) # Give time for the file(s) to save
...
def runOrbSLAM3Sequence(orbSlamLaunchConfig, orbSlamSequenceConfig):
    ...
    if (not orbSlamSequenceConfig.force_regenerate_results):
        if (checkForExistingResults(topLevelResultsDirForSeq, orbSlamSequenceConfig.generate_map_file, rosbagsSequence)):
    ...
    orbSaveService = rospy.ServiceProxy(ORBSLAM3RelatedConstants.orbSlamSaveTrajServiceName, OrbSLAMSaveFilesAndStartNewTrajectorySrv)
...
def orbSlam3SequenceArgParse():
    ...
    parser.add_argument(CmdLineArgConstants.prefixWithDashDash(CmdLineArgConstants.orbSlam3ConfigurationFileBaseArgName),
                        required=True,
    ...
    launchConfig = ORBSLAM3LaunchStartInfo(orb_slam_configuration_file=args_dict[CmdLineArgConstants.orbSlam3ConfigurationFileBaseArgName],
                                           orb_slam_vocabulary_file=args_dict[CmdLineArgConstants.orbSlam3VocabularyFileBaseArgName])

    sequenceConfig = ORBSLAM3SequenceConfig(sequence_dir=args_dict[CmdLineArgConstants.trajectorySequenceFileDirectoryBaseArgName],
