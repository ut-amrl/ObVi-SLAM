...
class ObjectMetricsForUTVSlamConfig:
    def __init__(self,
                 sequence_dir,
...
def generateMetricsForUTVSLAM(metricsConfig):
    metrics_for_approach_config = ObjectMetricsForApproachConfig(
        ...
        within_bagdir_sub_dir=FileStructureConstants.utVslamOutRootDirBaseName,
...
def objectMetricsForUTVSlamArgParse():
    parser = argparse.ArgumentParser(description="Generate object metrics for UT VSLAM sequence")
    ...
    return ObjectMetricsForUTVSlamConfig(
...
if __name__ == "__main__":
    objectMetricsConfig = objectMetricsForUTVSlamArgParse()
    generateMetricsForUTVSLAM(objectMetricsConfig)
