...
int main(int argc, char **argv) {
  ...
  for (const auto frame_id : ordered_frame_ids) {
    ...
    for (const auto &cam_id_and_image : cam_ids_and_images) {
      ...
      fs::path rel_image_path = fs::path(std::to_string(cam_id_and_image.first)) / (std::to_string(frame_id) + ".png");
      ...
      cam_ids_and_ofiles.at(cam_id_and_image.first) << rel_image_path.string() << std::endl;
    }
    ...
    for (const auto &cam_id_and_bboxes : cam_ids_and_bboxes) {
      fs::path rel_image_path = fs::path(std::to_string(cam_id_and_bboxes.first)) / (std::to_string(frame_id) + ".png");
      ...
      if (node_and_timestamp.node_id_ != frame_id) {
        LOG(ERROR) << "Something went wrong with node id and frame id! " 
            << "node id = " << node_and_timestamp.node_id_ << "; frame id = " << frame_id;
        exit(1);
      }
      ofile << "file_name" << std::endl;
      // ofile << node_and_timestamp.seconds_ << "_" << node_and_timestamp.nano_seconds_ << ".png" << std::endl;
      // ofile << std::to_string(frame_id) << ".png" << std::endl;
      ...
      for (const auto &bbox : cam_id_and_bboxes.second) {
        ...
        ofile << class_names_and_class_ids.at(bbox.semantic_class_) << std::endl;
